...
class AuthController extends Controller
{
    ...
    public function registerMentor(Request $request)
    {
        ...
        // Set default status so mentor is visible to startups
        ...
        return redirect()->route('login')->with('success', 'Mentor registered successfully! Please log in.');
    }
    ...
    public function login(Request $request)
    {
        ...
        $request->validate([
            ...
            'role'     => 'required|in:startup,mentor',
        ]);
        ...
        // Look up user in the appropriate table based on role
        if ($role === 'startup') {
        ...
        if (!$user || !Hash::check($password, $user->password)) {
            // Redirect back with error (Unit IV - session flash)
        ...
        $request->session()->put('user_name', $role === 'startup' ? $user->founder_name : $user->name);
